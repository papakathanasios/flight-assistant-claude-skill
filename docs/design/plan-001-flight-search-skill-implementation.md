...
**Goal:** Create a Claude Code skill (`/flight-search`) that guides users through 8 flight cost optimization strategies with web search and produces a consolidated report.

**Architecture:** Single SKILL.md prompt file in `skills/flight-search/`. The skill uses a state-machine conversation flow (intake -> filter -> strategies -> report). No application code -- purely prompt-driven, leveraging Claude's WebSearch, WebFetch, and Write tools.

**Tech Stack:** Claude Code skill framework (markdown prompt), WebSearch/WebFetch for real-time data, Write tool for report generation.
...
| File | Responsibility |
|------|---------------|
| `skills/flight-search/SKILL.md` | Main skill file -- complete prompt with frontmatter, intake, filter, strategies, report template, error handling |
| `reports/` | Directory for generated flight search reports (created at runtime by the skill) |
| `CLAUDE.md` | Project-level Claude instructions -- document the skill |
| `Issues - Pending Items.md` | Project issues/pending items tracker |
...
**Files:**
- Create: `skills/flight-search/SKILL.md` (empty placeholder)
- Create: `reports/.gitkeep`
- Create: `CLAUDE.md`
- Create: `Issues - Pending Items.md`

- [ ] **Step 1: Create skill directory and reports directory**

```bash
mkdir -p skills/flight-search
mkdir -p reports
```
...
- [ ] **Step 3: Create SKILL.md with frontmatter only**

Create `skills/flight-search/SKILL.md`:
...
- [ ] **Step 5: Create Issues - Pending Items.md**
...
```bash
git init
git add skills/flight-search/SKILL.md reports/.gitkeep CLAUDE.md "Issues - Pending Items.md"
git commit -m "chore: initialize project structure for flight-search skill"
```

---

### Task 2: Write skill intake phase

**Files:**
- Modify: `skills/flight-search/SKILL.md`

- [ ] **Step 1: Add intake phase to SKILL.md**

Append to `skills/flight-search/SKILL.md` after the frontmatter:
...
```bash
cat skills/flight-search/SKILL.md | head -5
```

Expected: frontmatter followed by the skill content.

- [ ] **Step 3: Commit**

```bash
git add skills/flight-search/SKILL.md
git commit -m "feat: add intake phase to flight-search skill"
```

---

### Task 3: Write relevance filter

**Files:**
- Modify: `skills/flight-search/SKILL.md`

- [ ] **Step 1: Add relevance filter to SKILL.md**

Append to `skills/flight-search/SKILL.md`:
...
```bash
git add skills/flight-search/SKILL.md
git commit -m "feat: add relevance filter and plan confirmation to flight-search skill"
```

---

### Task 4: Write strategies 1-4

**Files:**
- Modify: `skills/flight-search/SKILL.md`

- [ ] **Step 1: Add Strategy 1 (Ideal Date Scanner)**
...
- [ ] **Step 2: Add Strategy 2 (Hidden Flight Finder)**
...
- [ ] **Step 3: Add Strategy 3 (Route Optimizer)**
...
- [ ] **Step 4: Add Strategy 4 (Deals & Promotions Detector)**
...
```bash
git add skills/flight-search/SKILL.md
git commit -m "feat: add strategies 1-4 to flight-search skill"
```

---

### Task 5: Write strategies 5-8

**Files:**
- Modify: `skills/flight-search/SKILL.md`

- [ ] **Step 1: Add Strategy 5 (Fee Breakdown & Elimination)**
...
- [ ] **Step 2: Add Strategy 6 (Price Negotiation Email)**
...
- [ ] **Step 3: Add Strategy 7 (Flexibility & Risk Analysis)**
...
- [ ] **Step 4: Add Strategy 8 (Hidden Destination Tickets / Skiplagging)**
...
```bash
git add skills/flight-search/SKILL.md
git commit -m "feat: add strategies 5-8 to flight-search skill"
```

---

### Task 6: Write report generation and error handling

**Files:**
- Modify: `skills/flight-search/SKILL.md`

- [ ] **Step 1: Add report generation state**
...
[Include ONLY strategies that were executed. Copy the output from each strategy conversation, cleaned up for readability. Each strategy section should be self-contained.]
...
- [ ] **Step 2: Add error handling and guardrails**
...
```bash
git add skills/flight-search/SKILL.md
git commit -m "feat: add report generation and error handling to flight-search skill"
```

---

### Task 7: Final validation and documentation update

**Files:**
- Modify: `CLAUDE.md` (verify documentation is complete)
- Modify: `Issues - Pending Items.md`

- [ ] **Step 1: Read the complete SKILL.md and verify consistency**

```bash
wc -l skills/flight-search/SKILL.md
```

Check for:
- Frontmatter is valid (name, description fields present)
- All 8 strategies are present and numbered consistently
- State machine flow is complete (INTAKE -> CONFIRM_TRIP -> RELEVANCE_FILTER -> CONFIRM_PLAN -> STRATEGY_1..8 -> REPORT -> DONE)
- No placeholder text (TBD, TODO, etc.)
- Web search queries use consistent variable naming ([ORIGIN], [DEST], etc.)
- Output tables are consistent across strategies

- [ ] **Step 2: Update Issues - Pending Items.md**
...
1. **Manual testing needed** - The flight-search skill needs to be tested with real trip scenarios (round-trip, one-way, multi-city) to verify conversation flow and web search effectiveness.
2. **Web search reliability** - Some flight search queries may not return structured pricing data from Google Flights or Skyscanner. May need to refine search query patterns after testing.
3. **Report file naming for multi-city** - The naming convention for multi-city trips with 3+ legs may produce long filenames. Consider truncation strategy if needed.
...
```bash
git add CLAUDE.md "Issues - Pending Items.md"
git commit -m "docs: update project documentation and pending items"
```
...
Invoke `/flight-search` in Claude Code and verify:
- The skill loads without errors
- The intake phase starts by asking trip type
- Multiple choice options are presented

- [ ] **Step 2: Test round-trip flow**

Walk through a sample scenario:
- Round-trip, ATH -> LHR, June 15-22, +/- 3 days, 500 EUR, 1 adult, economy
- Verify relevance filter correctly identifies applicable strategies
- Verify at least Strategy 1 and 2 produce meaningful output with web search
- Verify report is generated to the correct path

- [ ] **Step 3: Note any issues found**

Add any issues discovered during testing to `Issues - Pending Items.md` and commit.

```bash
git add "Issues - Pending Items.md"
git commit -m "docs: update pending items from smoke test"
```
